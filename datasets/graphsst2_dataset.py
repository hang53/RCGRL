...
def get_dataloader(dataset, batch_size, degree_bias=True, data_split_ratio=[0.8, 0.1, 0.1], seed=2):
    ...
    #                                      generator=torch.Generator().manual_seed(seed[0])) # GH: seed[0] need to change to seed
        ...
        num_train = int(data_split_ratio[0] * len(dataset_n))
        num_eval = int(data_split_ratio[1] * len(dataset_n))
        num_test = len(dataset_n) - num_train - num_eval

        train, eval, test = random_split(dataset_n, lengths=[num_train, num_eval, num_test],
                                         generator=torch.Generator().manual_seed(seed[0])) # GH: seed[0] need to change to seed
...
if __name__ == '__main__':
    get_dataset(dataset_dir='./datasets', dataset_name='Graph_SST2')